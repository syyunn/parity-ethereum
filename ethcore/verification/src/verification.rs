...
pub fn verify_block_unordered(block: Unverified, engine: &dyn Engine, check_seal: bool) -> Result<PreverifiedBlock, Error> {
	...
	// check if nonce is too high
	if header.number() >= engine.params().dust_protection_transition {
...
pub fn verify_block_family<C: BlockInfo + CallContract>(
	...
	for tx in &block.transactions {
		...
		engine.machine().verify_transaction(tx, parent, client)?;
	}
...
pub fn verify_block_final(expected: &Header, got: &Header) -> Result<(), BlockError> {
	...
	if expected.log_bloom() != got.log_bloom() {
		return Err(BlockError::InvalidLogBloom(From::from(Mismatch {
...
pub(crate) fn verify_header_params(header: &Header, engine: &dyn Engine, check_seal: bool) -> Result<(), BlockError> {
	if check_seal {
	...
	if let Some(ref ext) = engine.machine().ethash_extensions() {
		if header.number() >= ext.dao_hardfork_transition &&
			...
			return Err(BlockError::ExtraDataOutOfBounds(From::from(OutOfBounds {
				min: None,
				max: None,
				found: 0
			})));
...
pub(crate) fn verify_header_time(header: &Header) -> Result<(), BlockError> {
	...
	if timestamp > invalid_threshold {
		return Err(BlockError::InvalidTimestamp(From::from(OutOfBounds {
			max: Some(max_time),
			min: None,
			found: timestamp
		})));
	...
	if timestamp > max_time {
		return Err(BlockError::TemporarilyInvalid(From::from(OutOfBounds {
			max: Some(max_time),
			min: None,
			found: timestamp
		})));
...
fn verify_parent(header: &Header, parent: &Header, engine: &dyn Engine) -> Result<(), BlockError> {
...
fn verify_block_integrity(block: Unverified) -> Result<Unverified, Error> {
